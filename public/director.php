...
if (empty($directorScripts)) {
    $brief = $settingsModel->get('director_brief', 'You have one actor, one phone camera, and a single location. Shoot a 60-second scene that tells a complete emotional story. Include your framing choices in your submission notes.');
    $directorScripts = [
        ['id'=>'scene','title'=>'Scene Direction','content'=>$brief,'audition_type'=>'Scene Direction','difficulty'=>'intermediate','duration_hint'=>'60-90 seconds','image_url'=>'','rules'=>"Video under 5 minutes\nShoot on any device\nExplain your framing choices\nOne take preferred\nDirector may be heard, not seen"],
        ['id'=>'pitch','title'=>'Vision Pitch','content'=>'Record a 60-second pitch on camera. Tell us about a scene you\'d love to direct — the mood, the visuals, the performance. Show us how you think as a filmmaker.','audition_type'=>'Vision Pitch','difficulty'=>'beginner','duration_hint'=>'60 seconds','image_url'=>'','rules'=>"Under 60 seconds\nShoot on any phone\nNo props needed\nJust speak to camera\nBe specific and passionate"],
    ];
}
...
<nav class="fp-nav">
  <div style="max-width:1200px;margin:0 auto;padding:0 1.25rem;height:100%;display:flex;align-items:center;justify-content:space-between">
    <a href="/" style="display:flex;align-items:center;gap:8px;text-decoration:none">
      <?php if ($logoUrl): ?><img src="<?= htmlspecialchars($logoUrl) ?>" alt="Faceless Pictures 3" style="height:30px;width:auto">
...
<section style="padding:5rem 1.25rem 2rem;text-align:center" class="fade-up">
  ...
  <h1 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(48px,8vw,80px);letter-spacing:.02em;line-height:.92;color:#111;margin-bottom:.75rem">DIRECTOR<br>AUDITIONS</h1>
  <p style="color:#6b7280;font-size:.9rem;max-width:380px;margin:0 auto;line-height:1.6">Pick a format. Read the brief. Shoot your video. Submit.</p>
</section>
<section style="padding:0 1.25rem 5rem">
  <div style="max-width:1200px;margin:0 auto">
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:1.5rem">
    <?php foreach ($directorScripts as $i => $script):
        $atype=$script['audition_type']??'Scene Direction';$diff=$script['difficulty']??'beginner';$dc=$diffColors[$diff]??$diffColors['beginner'];
        $rules=$script['rules']??"Video under 5 minutes\nShoot on any device\nExplain your creative choices";
        $ruleList=array_filter(array_map('trim',explode("\n",$rules)));
        $scriptId=is_numeric($script['id'])?(int)$script['id']:0;
    ?>
    <div class="script-card" x-data="auCard(<?= $scriptId ?>,<?= json_encode($atype) ?>,'director',<?= json_encode($script['title']) ?>,<?= json_encode($script['content']) ?>)">
      ...
      <div class="card-body">
        ...
        <div>
          ...
          <div class="script-text" :class="expanded?'expanded':''">  <?= nl2br(htmlspecialchars($script['content'])) ?></div>
          <button class="read-more-btn" @click="expanded=!expanded" x-text="expanded?'Show less ↑':'Read more ↓'"></button>
        </div>
        ...
        <div>
          ...
          <form @submit.prevent="submit" style="display:flex;flex-direction:column;gap:.625rem">
            ...
            <div><label class="fp-label">Notes <span style="text-transform:none;font-weight:400;color:#9ca3af">(optional)</span></label><textarea x-model="form.notes" class="fp-input" rows="2" placeholder="Framing choices, style notes..." style="resize:vertical"></textarea></div>
            <div class="card-divider"></div>
            <div>
            ...
            <button type="submit" class="btn-submit" :disabled="loading"><span x-show="!loading">Submit <?= htmlspecialchars($atype) ?> →</span><span x-show="loading">Uploading...</span></button>
          </form>
        </div>
      </div>
    </div>
