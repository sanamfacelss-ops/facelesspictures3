...
$fieldNameLabel = setting('director_field_name_label', 'Name *');
$fieldNamePlaceholder = setting('director_field_name_placeholder', 'Your full name');
$fieldEmailLabel = setting('director_field_email_label', 'Email *');
$fieldEmailPlaceholder = setting('director_field_email_placeholder', 'you@email.com');
$fieldPhoneLabel = setting('director_field_phone_label', 'Phone *');
...
function directorSubmit(){
    var component = {
        ...
        openTermsModal(){
            ...
            window.directorSubmitFn = this.submit.bind(this);
            // Show terms modal
...
function acceptTermsAndSubmit() {
    ...
    if (window.directorSubmitFn) {
        window.directorSubmitFn();
    } else {
...
function downloadSong(songs){
    ...
    if(songs.length === 1){
        // Find the film-song "Download Song" button for loader
        var btns = document.querySelectorAll('.film-song-btn');
        var targetBtn = null;
        btns.forEach(function(b){ if(b.textContent.trim().indexOf('Download') !== -1) targetBtn = b; });
